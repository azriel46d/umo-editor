  ...
  <div v-if="!page.preview?.enabled" class="umo-status-bar">
    <div class="umo-status-bar-left">
      <tooltip :content="page.showToc ? t('toc.hide') : t('toc.show')">
        <t-button class="umo-status-bar-button" :class="{ active: page.showToc }" variant="text" size="small"
      ...
      <tooltip v-if="options.document?.enableSpellcheck" :content="$document?.enableSpellcheck
          ...
          : t('spellcheck.enable')
        ">
        <t-button class="umo-status-bar-button" :class="{ active: $document.enableSpellcheck }" variant="text"
          size="small" @click="toggleSpellcheck">
    ...
    <div class="umo-status-bar-right">
      <tooltip :content="`${page.preview?.enabled ? t('preview.disable') : t('preview.title')} (F5)`">
        <t-button class="umo-status-bar-button" :class="{ active: page.preview?.enabled }" variant="text" size="small"
          @click="togglePreview">
          <icon name="preview" />
        </t-button>
      </tooltip>
      <tooltip :content="`${fullscreen?.isFullscreen ? t('fullscreen.disable') : t('fullscreen.title')} (F11 / ⌘+F11)`">
        <t-button class="umo-status-bar-button" variant="text" size="small" @click="fullscreen?.toggle">
          <icon :name="fullscreen?.isFullscreen ? 'full-screen-exit' : 'full-screen'
            " />
        </t-button>
      </tooltip>
      <div class="umo-status-bar-split"></div>
